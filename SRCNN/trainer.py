...
if config.exp_type == 0:
    ...
    from models.model_SRCNN import Model_Train
    config.model_tag = "SRCNN_model_default"

def generate_expname_automatically():
    name = "SRCNN_%s_%02d_%02d_%02d_%02d_%02d" % (config.model_tag,
...
while True : #manuallry stopping
    ...
    if step % 1 == 0:
        ...
        cv2.imshow('result_train_{}_{}'.format(config.exp_type,config.model_tag), resize(result_concat[...,::-1][0],1.5))
        cv2.waitKey(10)
    ...
    if step % 10 == 0:
        ...
        cv2.imshow('result_test_{}_{}'.format(config.exp_type,config.model_tag), resize(result_concat[...,::-1][0],1.5))
        cv2.waitKey(10)
