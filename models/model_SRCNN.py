...
from SRCNN.utils.data_loader import Dataset_Loader
import numpy as np
...
class Model_Train():
    ...
    def build_model(self):
        ...
        """ optimizer """
        var_list1 = [var for var in t_vars if "patch_extraction" in var.name or "non_linear_mapping" in var.name]
        var_list2 = [var for var in t_vars if "reconstruction" in var.name]
        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        with tf.control_dependencies(update_ops):
            train_op1 = tf.train.AdamOptimizer(self.config.learning_rate).minimize(loss, var_list=var_list1)
            train_op2 = tf.train.AdamOptimizer(self.config.learning_rate_sub).minimize(loss, var_list=var_list2)
            G_op = tf.group(train_op1, train_op2)
    ...
    def init_model(self):
        model_saver = tf.train.Saver(max_to_keep=10000)
        init = tf.global_variables_initializer()
        self.sess.run(init)
        #step = restore_model(FLAGS, sess)
